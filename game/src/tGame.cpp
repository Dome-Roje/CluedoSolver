...
tCard* tGame::getCard(eGuest& guest) {
    for(tCard* card : m_deck) {
        if(card->m_type == tCard::eType::guest && card->m_value.m_guest == guest) return card;
    }
    return NULL;
}
...
tCard* tGame::getCard(eRoom& room) {
    for(tCard* card : m_deck) {
        if(card->m_type == tCard::eType::room && card->m_value.m_room == room) return card;
    }
    return NULL;
}
...
tCard* tGame::getCard(eWeapon& weapon) {
    for(tCard* card : m_deck) {
        if(card->m_type == tCard::eType::weapon && card->m_value.m_weapon == weapon) return card;
    }
    return NULL;
}
...
void tGame::endGame() {
    ...
    for(tPlayer* player : m_players) {
        if(player->isAlive()) numberOfAlivePlayers++;
    }
...
tPlayer* tGame::playerAfter(tPlayer* player) {
    if(player == m_players.back()) return m_players.front();
    return player++;
}
...
void tGame::killPlayer(tPlayer& player) {
    ...
    if(numberOfAlivePlayers == 0) endGame();
}
